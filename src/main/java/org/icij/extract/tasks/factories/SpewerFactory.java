...
public abstract class SpewerFactory {
	...
	public static Spewer createSpewer(final Options<String> options) throws ParseException {
		final OutputType outputType = options.get("output-type").parse()
				.asEnum(OutputType::parse).orElse(OutputType.STDOUT);
		final Spewer spewer;

		if (OutputType.SOLR == outputType) {
			spewer = createSolrSpewer(options);
		} else if (OutputType.FILE == outputType) {
			spewer = new FileSpewer(options);
		} else {
			spewer = new PrintStreamSpewer(System.out, options);
		}
	 ...
	 * @return A new spewer configured according to the given parameters.
	 */
	private static SolrSpewer createSolrSpewer(final Options<String> options) {
		...
		return new SolrSpewer(new HttpSolrClient.Builder(options.get("index-address").value().orElse
