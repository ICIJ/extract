...
/**
 * A {@link FilterInputStream} that URL-encodes arbitrary binary data from an
 * input stream.
 *
 * No transcoding is performed on the input. If your input is UTF-16LE,
 * the output should be read as percent-encoded UTF-16LE. If your
 * input if UTF-8, the output will be percent-encoded UTF-8, and so on.
 *
...
public class URLEncodingInputStream extends FilterInputStream {
	...
	public static Reader createReader(final InputStream in) {
	...
	protected void encode(final int b) {
		if (' ' == b) {
			offset = 2;
			buffer[offset] = (int) '+';
