...
public class DocumentConsumerErrorTest {
	...
	public void testFatalErrorPropagatesToUncaughtHandler() throws Exception {
		final AtomicReference<Throwable> uncaught = new AtomicReference<>();
		// The latch lets us wait for the uncaught-exception handler to actually run,
		// which happens after the thread's run() completes — potentially after
		// awaitTermination returns, making a latch the only race-free sync point.
		final CountDownLatch handlerFired = new CountDownLatch(1);
		final java.util.concurrent.ExecutorService executor = newCapturingExecutor(uncaught, handlerFired);
		...
		// Block until the uncaught-exception handler has stored the throwable,
		// then clean up the executor.
		assertThat(handlerFired.await(5, TimeUnit.SECONDS)).isTrue();
		executor.shutdown();
	...
	private java.util.concurrent.ExecutorService newCapturingExecutor(final AtomicReference<Throwable> sink) {
		// No latch needed: the recoverable path never fires the handler, so null is stable
		// once awaitTermination confirms the task completed.
		return newCapturingExecutor(sink, null);
	}

	private java.util.concurrent.ExecutorService newCapturingExecutor(
			final AtomicReference<Throwable> sink,
			final CountDownLatch handlerFired) {
		final ThreadFactory factory = runnable -> {
			final Thread thread = new Thread(runnable);
			thread.setUncaughtExceptionHandler((t, e) -> {
				sink.set(e);
				// Signal that the handler has run so the waiting test can proceed.
				if (handlerFired != null) {
